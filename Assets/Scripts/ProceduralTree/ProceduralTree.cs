...
{
    [ExecuteInEditMode]
    public class ProceduralTree : TerrariumElement
    {
        ...
        public void GenerateTree()
        {
            ...
            if (vertexList == null
                || uvList == null
                || triangleList == null)
            { // Create lists for holding generated vertices
        ...
        {
            var progress = Mathf.Sqrt(Progress);
            ...
            { // After first base ring is added ...
            ...
            if (radius < MinimumRadius || vertexList.Count + NumberOfSides >= MaxNumVertices)
            { // End branch if reached minimum radius, or ran out of vertices
        ...
        void OnDisable()
        {
            ...
            if (PrefabUtility.GetPrefabType(this) == PrefabType.PrefabInstance)
            { // If it's a prefab instance, look for siblings
                var parentPrefab = PrefabUtility.GetPrefabParent(this);
                var list = (ProceduralTree[])FindObjectsOfType(typeof(ProceduralTree));
                foreach (var go in list)
                    if (go != this && PrefabUtility.GetPrefabParent(go) == parentPrefab)
